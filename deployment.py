...
def runContainerHelper(host_ip):
    ...
    try:
        ...
        if host_ip not in switch_list:
            switch_list.append(host_ip)
...
def runPigRelay(container):
    result = container.exec_run('sh -c "sed -i \'s/172.17.0.1/128.105.146.152/g\' pigrelay.py"')
